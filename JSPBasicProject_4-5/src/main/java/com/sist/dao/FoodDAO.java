...
public class FoodDAO {
	...
	public List<FoodVO> foodListData(int page) {
		...
		try {
			getConnection();
			String sql = "SELECT no,name,poster " + "FROM food " + "ORDER BY no ASC "
	...
	public List<FoodVO> foodFindData(int page, String address) {
		...
		try {
			getConnection();
			String sql = "SELECT no,name,poster " + "FROM food "
					+ "WHERE address LIKE '%'||?||'%' "
					+ "ORDER BY no ASC "
					+ "OFFSET ? ROWS FETCH NEXT 12 ROWS ONLY";
			ps = conn.prepareStatement(sql);
	...
	public int foodFindTotalPage(String address) {
		...
		try {
			getConnection();
			String sql = "SELECT CEIL(COUNT(*)/12.0) FROM food WHERE address LIKE '%'||?||'%'";
			ps = conn.prepareStatement(sql);
			ps.setString(1, address);
			ResultSet rs = ps.executeQuery();
			rs.next();
